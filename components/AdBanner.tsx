...
const AdBanner: React.FC<AdBannerProps> = ({ type }) => {
  const config = {
    sidebar: {
      ...
      title: STRINGS.ads.sidebarTitle,
      tag: STRINGS.ads.sidebarTag
    },
    horizontal: {
      ...
      title: STRINGS.ads.horizontalTitle,
      tag: STRINGS.ads.horizontalTag
    }
  };
  ...
  return (
    <div className={current.container}>
      <div className="flex-1 p-6 text-right">
        ...
        <button className="mt-2 text-[10px] font-black text-blue-600 bg-white px-4 py-1.5 rounded-full shadow-sm">{STRINGS.ads.learnMore}</button>
      </div>
